...
// 4. PROJECTOR OVERLAY (ZOOM IMAGE)
const ProjectorOverlay = ({ images, currentIndex, setCurrentIndex, onClose }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[60%] bg-[#ff5e00]/5 blur-[120px] animate-pulse" />
      </div>

      {/* BOUTON FERMER */}
      <button
        onClick={onClose}
        className="absolute top-6 right-6 z-[210] text-white/50 hover:text-[#ff5e00] transition-colors p-2"
      >
        <Power size={30} />
      </button>

      {/* NAVIGATION FLÈCHES (DESKTOP) */}
      <div className="absolute inset-x-4 top-1/2 -translate-y-1/2 flex justify-between z-[210] pointer-events-none">
        <button onClick={() => paginate(-1)} className="p-4 pointer-events-auto bg-black/50 border border-white/10 text-white hover:border-[#ff5e00] transition-all rounded-full group">
          <ChevronDown className="rotate-90 group-hover:-translate-x-1 transition-transform" />
        </button>
        <button onClick={() => paginate(1)} className="p-4 pointer-events-auto bg-black/50 border border-white/10 text-white hover:border-[#ff5e00] transition-all rounded-full group">
          <ChevronDown className="-rotate-90 group-hover:translate-x-1 transition-transform" />
        </button>
      ...
      {/* CONTAINER DE L'IMAGE AVEC SWIPE */}
      <div className="relative w-full max-w-5xl px-4 flex items-center justify-center">
        <AnimatePresence mode="wait">
          <motion.div
            key={currentIndex}
            layoutId={currentIndex === 0 ? `img-${activeImage.url}` : undefined} // LayoutId seulement sur la première pour l'effet projecteur
            drag="x"
            ...
            className="relative w-full aspect-video rounded-xl border border-[#ff5e00]/30 shadow-[0_0_50px_rgba(255,94,0,0.15)] overflow-hidden bg-black flex items-center justify-center cursor-grab active:cursor-grabbing"
          >
            <img
              src={activeImage.url}
            ...
            {/* HUD OVERLAY */}
            <div className="absolute top-4 left-6 font-mono text-[10px] text-[#ff5e00] flex flex-col gap-1">
              <span className="animate-pulse">● SIGNAL_PROJECTEUR: ACTIF</span>
              <span className="text-white/40 uppercase">DOSSIER: {activeImage.label}</span>
              <span className="text-white/20 uppercase">INDEX: {currentIndex + 1} / {images.length}</span>
            </div>

            <div className="absolute inset-0 pointer-events-none bg-gradient-to-b from-transparent via-[#ff5e00]/5 to-transparent h-20 w-full animate-[scan_4s_linear_infinite]" />
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      <div className="absolute bottom-10 flex gap-2">
        {images.map((_, i) => (
          <div
            key={i}
            className={`h-1 transition-all duration-300 rounded-full ${i === currentIndex ? 'w-8 bg-[#ff5e00]' : 'w-2 bg-white/20'}`}
          />
        ))}
      </div>
    </motion.div>
...
const ProjectImage = ({ src, alt, onExpand }) => (
  <motion.div
    layoutId={`img-${src}`}
    onClick={(e) => {
      e.stopPropagation();
    ...
    className="relative group overflow-hidden rounded-lg border border-white/10 bg-black cursor-zoom-in"
  >
    {/* Overlay de scanlines spécifique à l'image */}
    <div className="absolute inset-0 z-10 pointer-events-none opacity-20 bg-[repeating-linear-gradient(0deg,transparent,transparent_1px,#fff_2px)]"></div>

    {/* Filtre de couleur Teknika au survol */}
    <motion.div
    ...
    {/* Coins tactiques (Viseurs) */}
    <div className="absolute top-2 left-2 w-2 h-2 border-t border-l border-[#ff5e00]"></div>
    <div className="absolute top-2 right-2 w-2 h-2 border-t border-r border-[#ff5e00]"></div>
    <div className="absolute bottom-2 left-2 w-2 h-2 border-b border-l border-[#ff5e00]"></div>
    <div className="absolute bottom-2 right-2 w-2 h-2 border-b border-r border-[#ff5e00]"></div>
  </motion.div>
);

const ProjectFile = ({ title, type, context, stack, result, images = [], onImageClick }) => {
  const [isOpen, setIsOpen] = useState(false);
  ...
  return (
    <div className="mb-8 relative pl-6 group">
      ...
      <div onClick={handleToggle} className={`relative overflow-hidden rounded-xl transition-all duration-300 cursor-pointer border-2 ${isOpen ? 'bg-black/95 border-[#ff5e00]' : 'bg-black/60 border-white/40 hover:border-white/80 hover:bg-black/80'}`}>

        {/* Header du Projet */}
        <div className="p-5 flex justify-between items-start">
          <div>
            <h3 className="text-xl font-bold text-white uppercase tracking-wider">{title}</h3>
            <p className="text-[10px] font-mono text-[#0d47ff] mt-1 tracking-[0.3em]">{type}</p>
          </div>
            ...
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-white/10">
              <div className="p-5 grid md:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  <div>
                    <p className="text-[9px] text-gray-500 font-mono mb-2">/// ANALYSE_DU_PROBLÈME</p>
                    <p className="text-sm text-gray-300 font-mono leading-relaxed"><DecryptText text={context} start={isOpen} speed={10} /></p>
                  </div>
                  <div className="pl-4 border-l-2 border-[#ff5e00]">
                    <p className="text-[9px] text-[#ff5e00] font-mono mb-2">/// RÉSULTAT_DÉPLOYÉ</p>
                    <p className="text-sm text-white font-medium">{result}</p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {stack.map((tech, i) => (
                      <span key={i} className="text-[9px] font-mono px-2 py-1 bg-white/5 border border-white/10 text-gray-400">{tech}</span>
                    ))}
                  </div>
                </div>

                {/* COLONNE DROITE : VISIONNEUSE D'IMAGES */}
                {images.length > 0 && (
                  <div className="space-y-4">
                    <ProjectImage
                      src={images[activeImg].url}
                      alt={images[activeImg].label}
                      onExpand={() => onImageClick && onImageClick(images[activeImg], images)}
                    />

                    {/* Sélecteur de fichiers (miniatures ou liste) */}
                    <div className="flex flex-wrap gap-2">
                      {images.map((img, idx) => (
                        <button
                          key={idx}
                          onClick={(e) => { e.stopPropagation(); setActiveImg(idx); }}
                          className={`text-[9px] font-mono px-2 py-1 border transition-all ${activeImg === idx ? 'bg-[#ff5e00] border-[#ff5e00] text-black' : 'bg-transparent border-white/20 text-gray-500 hover:border-white/50'}`}
                        >
      ...
      <AnimatePresence>
        {galleryState.isOpen && (
          <ProjectorOverlay
            ...
            setCurrentIndex={(newIdx) => setGalleryState(prev => ({ ...prev, index: newIdx }))}
...
                /// PROFIL TEKNIKA
            ...
            <div className="max-w-3xl">
              ...
              <Reveal delay={0.1}>
                <ProjectFile
                  ...
                  images={[
                    { url: '/Horeb/Dashboard.png', label: 'Dashboard' },
                    { url: '/Horeb/connexion.png', label: 'Accès' },
                    { url: '/Horeb/BD.png', label: 'Base de Données' },
                    { url: '/Horeb/Comptabilité.png', label: 'Compta' },
                    { url: '/Horeb/rh.png', label: 'RH' },
                    { url: '/Horeb/devis.png', label: 'Facturation' },
                    { url: '/Horeb/devispdf.png', label: 'Export PDF' },
                  ]}
                  onImageClick={(img, allImages) => {
                    const idx = allImages.findIndex(i => i.url === img.url);
                    setGalleryState({ images: allImages, index: idx, isOpen: true });
                  }}
                />
              </Reveal>
          ...
          <section className="py-24 px-6 bg-black pb-40">
            <Reveal className="max-w-xl mx-auto bg-[#0a0a0a] border border-[#25D366]/20 p-8 rounded-2xl relative overflow-hidden text-center">
              ...
              {/* BOUTON WHATSAPP BLINDÉ */}
              <motion.div whileTap={{ scale: 0.95 }} className="w-full relative z-50">
                <a
                  href="https://wa.me/2250747640441?text=Bonjour%20Aristide,%20on%20peut%20parler%20affaire%20?"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-3 w-full py-4 bg-[#25D366] hover:bg-[#1ebc57] text-black font-bold text-lg rounded-xl shadow-[0_0_20px_rgba(37,211,102,0.3)] transition-all cursor-pointer no-underline select-none"
                  onClick={() => dispatchLog("Ouverture de WhatsApp...")}
                >
                  {/* Icône WhatsApp SVG inline pour être sûr qu'elle s'affiche */}
